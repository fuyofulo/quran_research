#!/usr/bin/env python3
"""
Build translation-divergence catalog for 11 success-vocabulary roots.

Outputs to: data/structural/success-divergence.json
"""
...
def build_divergence_catalog():
    """
    Load words.jsonl to identify all ayahs containing success roots.
    ...
    print(f"   Found {len(ayah_roots)} ayahs with success roots", file=sys.stderr)
    ...
    # Collect all success-ayahs with their divergence scores
    ...
    # Surah concentration: identify top 10 surahs by success-root density with high divergence
    surah_success_divergence = defaultdict(lambda: {'count': 0, 'total_divergence': 0.0, 'ayahs': []})

    for (surah, ayah), roots in ayah_roots.items():
        key = f"{surah}:{ayah}"
        if key in divergence_data:
            div_score = divergence_data[key]['scores']['combined']
            surah_success_divergence[surah]['count'] += 1
            surah_success_divergence[surah]['total_divergence'] += div_score
            surah_success_divergence[surah]['ayahs'].append({
                'ayah': ayah,
                'roots': sorted(list(roots)),
    ...
    surah_concentration = []
    for surah in sorted(surah_success_divergence.keys()):
        entry = surah_success_divergence[surah]
        avg_div = entry['total_divergence'] / entry['count'] if entry['count'] > 0 else 0.0
        surah_concentration.append({
            'surah': surah,
            'success_ayah_count': entry['count'],
    ...
    output = {
        ...
        'total_success_ayahs': len(ayah_divergence_list)
    }

    output_path = 'data/structural/success-divergence.json'
