#!/usr/bin/env python3
"""
Build co-occurrence matrix + translation-divergence catalog for 32 failure-vocabulary roots.
Mirrors methodology of success-cooccurrence.py.

Outputs to: data/structural/failure-cooccurrence.json

Failure roots (Buckwalter, case-sensitive):
  xsr, xyb, hlk, bwr, DyE, bTl, Dll, ErD, gfl, nks, jhl, kfr, nfq, $rk, jHd, kbr, Tgy, bgy, srf, fjr, qsw, xtm, qfl, mrD, glf, knn, *nb, Avm, Zlm, swA, fsq, fsd
"""
...
def build_cooccurrence_matrix():
    """
    Load words.jsonl, group by (surah, ayah), identify which failure roots appear in each ayah.
    Build 32x32 co-occurrence matrix.
    """
    ...
    print(f"   Found {len(ayah_roots)} ayahs containing failure roots", file=sys.stderr)

    # Initialize 32x32 matrix
    root_list = sorted(FAILURE_ROOTS)
    n = len(root_list)
    ...
    for ayah, roots in ayah_roots.items():
        roots_list = [r for r in roots if r in FAILURE_ROOTS]
    ...
    # Surah density: (failure-vocabulary ayahs in surah) / (all root-bearing words)
    print("[4/5] Computing surah density...", file=sys.stderr)

    surah_all_roots = defaultdict(int)
    surah_failure_roots = defaultdict(int)
    ...
    with open('data/morphology/words.jsonl', 'r') as f:
        for line in f:
            ...
            if root:
                ...
                if root in FAILURE_ROOTS:
                    surah_failure_roots[surah] += 1

    surah_density = {}
    for surah_num in range(1, 115):
        total = surah_all_roots.get(surah_num, 0)
        failure = surah_failure_roots.get(surah_num, 0)
        density = (failure / total * 100) if total > 0 else 0.0
    ...
    output_path = Path('data/structural/failure-cooccurrence.json')
